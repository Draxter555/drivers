...
```
> sudo ./app
пытаемся прочитать (будет ждать данные)...
```
> после в другом окне терминала отправляем данные:
```
> sudo chmod 666 /dev/pz4_dev
> echo "data" /dev/pz4_dev
```
> после в первом окне терминала получаем данные:
```
прочитано: data
```
